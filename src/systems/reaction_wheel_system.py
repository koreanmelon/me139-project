...
from systems.base_system import (BaseSystem, Joint, Link, MatDict, TCoordinate,
                                 TLine, VarDict, Vec)
...
class ReactionWheelSystem(BaseSystem):
    ...
    def __init__(self, l_1=1.0, l_c1=1.0, r=1.0, m_1=1.0, m_2=1.0) -> None:
        ...
        self.theta_d = {
            1: sp.diff(self.theta[1], BaseSystem.t_sym),
            2: sp.diff(self.theta[2], BaseSystem.t_sym)
        }
        self.theta_dd = {
            1: sp.diff(self.theta_d[1], BaseSystem.t_sym),
            2: sp.diff(self.theta_d[2], BaseSystem.t_sym)
        }
        
        self.Q = sp.Matrix([self.theta[1], self.theta[2]]).reshape(2, 1)
        
        self.P = sp.simplify((self.m[1] * BaseSystem.g_sym * l_c1 * sp.sin(self.theta[1])) + (self.m[2] * BaseSystem.g_sym * l_1 * sp.sin(self.theta[1])))
    ...
    def compute_T(alpha: VarDict, b: VarDict, theta: VarDict, d: VarDict) -> MatDict:
        T: dict[str, sp.Matrix] = {
            "1->0": BaseSystem.construct_T(alpha[1], b[1], theta[1], d[1]),
            "2->1": BaseSystem.construct_T(alpha[2], b[2], theta[2], d[2])
        }
        T |= {
    ...
    def compute_R(T: MatDict) -> MatDict:
        R: dict[str, sp.Matrix] = {
            "1->0": BaseSystem.extract_R(T["1->0"]),
            "2->1": BaseSystem.extract_R(T["2->1"]),
            "2->0": BaseSystem.extract_R(T["2->0"]),
        }
    ...
    def compute_torque(M: sp.Matrix, Q: sp.Matrix, V: sp.Matrix, G: sp.Matrix) -> sp.Matrix:
        Q_dd = Q.diff(BaseSystem.t_sym, 2)  # type: ignore
    ...
    def solve_system(self):
        ...
        self.sol_theta_1dd: Callable[[float, float, float, float, float, float], float] = sp.lambdify(
            ...
            sol[self.theta_dd[1]].subs(BaseSystem.g_sym, 9.81)
        )
        
        self.sol_theta_2dd: Callable[[float, float, float, float, float, float], float] = sp.lambdify(
            (self.theta[1], self.theta_d[1], self.theta[2], self.theta_d[2], tau_1, tau_2),
            sol[self.theta_dd[2]].subs(BaseSystem.g_sym, 9.81)
        )
    ...
    def deriv(self, t: Vec, Q: Vec) -> Vec:
        ...
        theta_1dd: float = self.sol_theta_1dd(theta_1, theta_1d, theta_2, theta_2d, 0, self.tau(Q))
        theta_2dd: float = self.sol_theta_2dd(theta_1, theta_1d, theta_2, theta_2d, 0, self.tau(Q))
